select p.PROIZVOD#,p.NAZIV_PROIZVODA,p.ROK_VAZENJA,p.DATUM_OTV,p.GRUPA_PROIZVODA#,p.RADNIK#,p.JEDINICA_MERE#, p.DODATNI_NAZIV ,p.KOD_KB,p.MIN_NIVO_ZALIHA,p.MAX_NIVO_ZALIHA, 
p.OPTIMALNI_NIVO_ZALIHA,p.JM_AKTIVITETA,p.KONTINUALNA_PR, p.SLEDLJIVOST,p.KOD_PAKOVANJA,p.FAMILIJA_REF,p.NAZIV_NA_ENGLESKOM,p.VREME_NABAVKE, p.VREME_KONTROLE,p.ORDER_MIN_KOLICINA, 
p.ORDER_MAX_KOLICINA,p.OVER_QUANTITY_CONTROL,p.DAYS_EARLY_RECEIPT,p.DAYS_LATE_RECEIPT,p.RECEIPT_DATE_CONTROL,p.OBRISAO_RADNIK#,p.DATUM_BRISANJA,p.VERSION_CONTROL,p.SERIAL_CONTROL, 
p.DATUM_KRAJA_AKCIJE,p.DATUM_DEKOR_MESECA, null slika, p.ID_JEDINICE_MERE_ALT_REF,  null cena, null dezenIstruktira, null proizvodjac, null tipAkcije, null kolicinaPoPakovanju, 
null raspolozivo, naziv_proizvoda||dodatni_naziv punNazivProizvoda, null maticnoSkladiste, null kolUAltJM, null stopaPoreza, k.klasifikacija# sortKlasa, null jedinicaMereRezervacije  
...
and ((sc.id = vrattr.wo_sort_per_class_id and p.proizvod# = vrattr.proizvod#)        
        ...
        and c.attribute_owner = 'CLASS')) 
order by  35 ASC, 41 DESC,1
